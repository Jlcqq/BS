      ...
      <el-dialog :title="data.title" width="60%" v-model="data.formVisible" :close-on-click-modal="false" destroy-on-close>
        <el-form :model="data.form" ref="formRef" :rules="data.rules" label-width="100px" style="padding-right: 50px;margin-top: 20px;">
          ...
          <el-form-item label="链接地址" prop="url">
            <el-input v-model="data.form.url" placeholder="请输入链接地址" autocomplete="off" />
          </el-form-item>
        </el-form>
        <template #footer>
...
//信息列表
const load =()=>{
  request.post('linksList', qs.stringify({
    key:data.key,
...
const save = () => {
  formRef.value.validate((valid=>{
    if (valid){
      request.post('linksAdd', qs.stringify({
        id:data.form.id,
        name:data.form.name,
        url:data.form.url,
      })).then(res => {
...
const Delete = (id) => {
  ElMessageBox.confirm('删除后数据无法恢复，您确定删除吗?', { type: 'warning' }).then(res => {
    request.post('linksDel?id=' + id).then(res => {
      if (res.code === '200') {
        load()
        ElMessage.success('操作成功')
      } else {
...
const delAll=()=>{
  if(data.ids.length === 0){
    ...
    ElMessageBox.confirm('删除后数据无法恢复，您确定删除吗?', { type: 'warning' }).then(res => {
      ...
      request.post('linksDeleteAll',qs.stringify({
        vals:data.ids
      })).then(res => {
        if (res.code === '200') {
          load()
